...
class FNAgent:
    ...
    def load(cls, env, model_path, epsilon=0.0001):
        ...
        # Note: You need to initialize the model first before loading
        # This is a placeholder - actual implementation depends on model architecture
        raise NotImplementedError("Model architecture must be defined before loading")
...
class Logger:
    def __init__(self, log_dir="", dir_name=""):
        ...
        # wandbの初期化（必要に応じて外部で行うことも可能）
        if not wandb.run:
            wandb.init(project="pytorch-rl", dir=self.log_dir)
    ...
    def write_image(self, index, frames):
        ...
        # wandbに画像をログ（複数フレームを並べて表示）
        if processed_frames:
            wandb.log(
                {tag: [wandb.Image(frame) for frame in processed_frames]}, step=index
            )
